      ...
      <li class="forumpost<?php if ($assoc_id && !$artwork) echo ' forumpostattachment'; if ($compact) echo ' compact'; ?>">
        ...
        <?php roots_post_inside_before(); ?>
             <div class="entry-content">
            	 <h5><?php eas_display_avatar($the_post->post_author); ?><?php the_author_meta('nickname'); ?><span class="subject"><?php the_title(); ?></span></h5>
            <?php if (!$edit && !$compact) {
            	the_content();
            } ?>
          <?php if (!$edit) { ?>
          <?php if (!$compact) { ?>
            ...
            <footer> <?php eas_forum_meta(); ?>
              <?php eas_new_post_button(true, true); ?>
              <?php eas_edit_button(); ?>
	...
	$the_return = '<form ';
		...
		$the_return .='action="'.$action.'" method="post" id="'.$elid.'">';
         ...
         $the_return.='   <p><input type="text" name="title" placeholder="Title" value="'.$title.'"></p>
            <p><input type="text" name="year" placeholder="Year" value="'.$year.'"></p>
            ...
            if (count($nudge)) {
            	...
            	$the_return.='	<p>The following information isn\'t strictly necessary to submit art but it helps us determine if your work is right for the site (it most likely is!). You can change this information in your settings.</p>';
            	
            }            	

            foreach ($nudge as $n) {
            	$the_return.='
            		<p><input type="text" name="'.strtolower($n).'" placeholder="Your '.$n.'" value=""></p>
            	';
            }              
...
function eas_get_cc_settings($heading = 'Creative Commons License', $explain = true) {
    	...
    	if ($explain) {
    		$the_return .= '
    	<p>These settings let people know how your work should be shared. You can read more about them on the <a href="http://creativecommons.org/licenses/" title="About the Licenses - Creative Commons">Creative Commons</a> site.</p>
...
function eas_new_post_button($mini = false, $reply = false) {
	...
	?>
		<a href="<?php echo $url; ?>" class="btn<?php echo $class_suffix; ?>">
			<?php echo $text; ?>
		</a>
	<?php
}
...
function eas_forum_meta() {
	...
	echo '<time class="updated" datetime="'. get_the_time('c') .'" pubdate>'. sprintf($link_before.'%s'.$link_after, wp_days_ago()) .'</time>';
	eas_forum_cat();
...
function eas_recently_updated_artists($page = 0, $per_page = 10, $contest = false) {
	global $wpdb;
	$contestname = false;
	if ($contest !== false) {
		$contestname = $contest;
...
function eas_figure_tag($id, $size = 'medium', $contest = false) {
	echo eas_get_figure_tag($id, $size);
}

function eas_get_figure_tag($id, $size = 'medium') {
		if ($size == 'thumb') {
		$size = 'thumbnail';
		$sizeclass = 'thumb';
	} else {
...
function eas_get_homebg() {
	global $wpdb;
	$bgs = $wpdb->get_col("select p.ID from wp_postmeta m
		join wp_posts p
		on p.ID = m.post_id
